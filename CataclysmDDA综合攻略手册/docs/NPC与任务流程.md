...
狩猎动物、采集植物、设置陷阱、收集材料和收集柴火任务，均属于**资源采集型任务**，其收获与任务物品组、NPC的技能和属性、任务难度、硬编码的生成数量有关；

当从资源采集型任务召回NPC时，会尝试从任务物品组中刷新一定量的物品，尝试次数为（实际任务时间➗最短任务时间）取整的倍数，具体倍数和物品组取决于任务类型，其中狩猎动物的倍数为2，设置陷阱的倍数为4，收集材料或柴火的倍数为6，采集植物的倍数为12；

每种任务均有一定的难度，其中狩猎动物或设置陷阱为30，其余为15；

根据具体任务类型，选择NPC的某些技能和属性计算得到综合技能点，具体计算方式如下：**狩猎动物**为（1.5×「枪法」等级+0.5×「感知」），**设置陷阱**为（（2×「操纵装置」等级+「感知」），其余为（2×「生存」等级+「感知」）；

每次尝试刷出物品时，会生成一个（1至任务难度）的随机数，若该数小于综合技能点，则成功刷出，否则本次尝试失败；

重点介绍狩猎动物和设置陷阱的产出池子：狩猎动物的池子相当于怪物组“GROUP_CAMP_HUNTING”与“GROUP_CAMP_HUNTING_LARGE”合并，而设置陷阱的池子相当于怪物组“GROUP_CAMP_HUNTING”与“GROUP_CAMP_TRAPPING”合并。（这种C++与JSON混合的写法实在令人费解）
